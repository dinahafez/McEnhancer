...
for i in range(0,len(sequenceSetDict)):
	...
	file1HeaderLine = compareFile1.readline();
	compareList1 = list();
	for line in compareFile1:
	...
	for j in currComparisonList:
		...
		for iter in range(0,iterations):
			...
			for k in range(0,cvFold):
				...
				auROCMax = 0;
				lambdaInitial = 0.001;
				for lambdaMult in range(0,0):
					lambdaThis = lambdaInitial + (float(lambdaMult)*(lambdaInitial));
					runTrainCmd = "/data/ohler/Dina/Drosophila/Research/code/l1_logreg/l1_logreg_train -q -s " + trainFeatureOutfileName + " " + trainClassOutfileName + " " + str(lambdaThis)  + " " + modelFileName +" > train.out";
			...
			#	runAuROCCmd = "/usr/bin/R CMD BATCH --slave "--args $"labelPredFileName + " $" + performanceFileName + " getAuROC.r > t.o";
